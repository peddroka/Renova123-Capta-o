# Graph Report - Renova123 Captação  (2026-08-08)

## Corpus Check
- 155 files · ~79,308 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1280 nodes · 2091 edges · 113 communities (96 shown, 17 thin omitted)
...
1. `MemoryRepository` - 34 edges
2. `MockWhatsAppProvider` - 32 edges
3. `EvolutionWhatsAppProvider` - 30 edges
4. `SupabaseRepository` - 29 edges
5. `Repository` - 28 edges
6. `buildApp()` - 25 edges
7. `WhatsAppProvider` - 25 edges
8. `api()` - 21 edges
9. `AiDecision` - 17 edges
10. `GroqProvider` - 16 edges
...
- `processInboundEvent()` --calls--> `normalizeBrazilianPhone()`  [EXTRACTED]
  apps/worker/src/index.ts → packages/core/src/phone.ts
- `processOutbound()` --calls--> `canStartOutreach()`  [EXTRACTED]
  apps/worker/src/index.ts → packages/core/src/schedule.ts
- `deliverGroupNotification()` --calls--> `canAttemptGroupDelivery()`  [EXTRACTED]
  apps/worker/src/index.ts → packages/core/src/group-notifications.ts
...
Cohesion: 0.11
Nodes (35): acquireInstanceLock(), applyFollowUpDecision(), configuredGroq(), conversationKey(), DeferredJobError, deliverGroupNotification(), executeAgent(), geminiCircuit (+27 more)

### Community 2 - "core/src/index.ts"
Cohesion: 0.27
Nodes (6): safeSlotReply(), validateCommercialClaims(), AppointmentTool, HandoffTool, QualificationService, AiDecision
...
Cohesion: 0.05
Nodes (19): canonicalQueue(), EditableResourceKey, editableTable, jobPriority(), MemoryRepository, mockRows, OutreachCapacity, PageResult (+11 more)
...
### Community 7 - "app.ts"
Cohesion: 0.08
Nodes (35): buildApp(), creatableResources, createAuthClient(), createServiceClient(), deletableResources, editableResourceKey(), ensureNoAppointmentConflict(), fastify (+27 more)

### Community 8 - "group-notifications.ts"
Cohesion: 0.27
Nodes (13): enqueueGroupNotification(), markStalledLead(), notifySalesQualified(), processQualificationDeadlines(), canAttemptGroupDelivery(), field(), format(), formatQualifiedGroupMessage() (+5 more)

### Community 9 - "groq.ts"
Cohesion: 0.07
Nodes (23): AgentCallMetrics, estimate(), GeminiProvider, GeminiProviderError, GeminiRateLimitError, metrics(), callMetrics(), estimateTokens() (+15 more)

### Community 10 - "index.tsx"
Cohesion: 0.10
Nodes (23): Theme, Item, KnowledgePage(), Material, MaterialsPage(), Result, AppShell(), ConfirmDialog() (+15 more)

### Community 11 - "api"
Cohesion: 0.11
Nodes (19): api(), clearToken(), getToken(), requestToken(), Feedback(), SkeletonTable(), PageHeader(), AiStatus (+11 more)
...
Cohesion: 0.12
Nodes (15): groq-sdk, dependencies, groq-sdk, @renova123/shared, zod, exports, @renova123/shared, zod (+7 more)
...
### Community 22 - "ProviderCircuitBreaker"
Cohesion: 0.26
Nodes (4): configuredGemini(), executeAgentWithDailyLimitFallback(), CircuitState, ProviderCircuitBreaker

### Community 23 - "csv.ts"
Cohesion: 0.22
Nodes (12): acceptedHeaders, cleanHeader(), CsvPreviewRow, guessDelimiter(), parsePhoneList(), splitLine(), expandScientificNotation(), invalid() (+4 more)
...
Cohesion: 0.20
Nodes (13): deliveryStatus(), detectMessageType(), eventTime(), extractText(), headerValue(), ignoreReason(), IntegrationError, normalizeConnectionState() (+5 more)

### Community 28 - "AppointmentsPage.tsx"
Cohesion: 0.15
Nodes (17): Appointment, AppointmentModal(), AppointmentsPage(), dateKey(), label(), local(), monthDays(), statuses (+9 more)
...
### Community 31 - "PageHeader.tsx"
Cohesion: 0.38
Nodes (5): navigation, NavigationItem, pageMeta, PageKey, HeroHeader()
...
### Community 33 - "conversation-style.ts"
Cohesion: 0.31
Nodes (12): completeClarificationReply(), containsBusinessFact(), continuationQuestion(), currentLeadTurn(), ensureConversationalProgress(), naturalMessageParts(), normalizeComparable(), removeRepeatedFormulations() (+4 more)
...
Cohesion: 0.18
Nodes (12): App(), resourceRoutes, AppLayout(), GroqPage(), ConnectionState, Diagnostics, formatDate(), labelState() (+4 more)
...
Cohesion: 0.20
Nodes (6): EvolutionWhatsAppProvider, isRecoverable(), recoverableStatus(), sendResult(), sleep(), WhatsAppSendResult
...
### Community 57 - "agent-context-builder.ts"
Cohesion: 0.22
Nodes (10): AgentContextBuilder, clipRecord(), normalize(), pick(), relevantKnowledge(), keywords(), KnowledgeService, normalize() (+2 more)

### Community 87 - "mock-whatsapp.ts"
Cohesion: 0.19
Nodes (7): EvolutionConfig, sanitizeWebhookPayload(), config, NormalizedWhatsAppEvent, NormalizedWhatsAppEventType, WhatsAppDownloadedMedia, WhatsAppMessageKey

### Community 88 - "README.md"
Cohesion: 0.15
Nodes (7): Arquitetura, Fluxo, Módulos finais, GroqCloud, Operação, Segurança, Comece aqui

### Community 89 - ".getConnectionStatus"
Cohesion: 0.29
Nodes (4): normalizeQr(), WhatsAppConnectionState, WhatsAppConnectionStatus, WhatsAppQrCode
...
Cohesion: 0.12
Nodes (18): SalesStageService, terminal, aggregateOutreachByHour(), localHour(), median(), OUTREACH_ANALYTICS_MIN_SAMPLE, OUTREACH_ANALYTICS_TIMEZONE, canStartOutreach() (+10 more)
...
### Community 97 - "conversation-memory-service.ts"
Cohesion: 0.43
...
### Community 117 - "francisco.test.ts"
Cohesion: 0.20
Nodes (5): baseSnapshot, AgentDecisionValidator, facts, snapshot, GroqModel
...
- **Why does `EvolutionWhatsAppProvider` connect `EvolutionWhatsAppProvider` to `worker/src/index.ts`, `MockWhatsAppProvider`, `app.ts`, `WhatsAppProvider`, `mock-whatsapp.ts`, `.getConnectionStatus`, `evolution.ts`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Why does `MemoryRepository` connect `MemoryRepository` to `Repository`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `PageKey` connect `PageHeader.tsx` to `App.tsx`, `MemoryRepository`, `app.ts`, `Repository`, `api`, `ResourcePage.tsx`, `shared/src/index.ts`?**
