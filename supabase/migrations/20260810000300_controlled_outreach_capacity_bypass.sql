...
create or replace function public.check_outreach_capacity(
  p_owner uuid,
  p_lead uuid,
  p_daily_limit integer,
  p_hourly_limit integer,
  p_allow_controlled_test_bypass boolean
)
returns jsonb language plpgsql security definer set search_path=public as $$
declare
  v_settings jsonb; v_enabled boolean; v_phone text; v_controlled boolean;
  v_timezone text; v_day date; v_now_local timestamp; v_start time; v_end time;
  v_start_local timestamp; v_end_local timestamp; v_campaign_start timestamptz;
  v_count integer; v_hour integer; v_next timestamptz; v_interval numeric;
  v_remaining integer;
begin
  select phone into v_phone from public.leads where owner_id=p_owner and id=p_lead;
  v_controlled:=coalesce(p_allow_controlled_test_bypass,false) and v_phone='5582988543864';
  select values into v_settings from public.app_settings where owner_id=p_owner and section='outreach';
  v_settings:=coalesce(v_settings,'{}'::jsonb); v_enabled:=coalesce((v_settings->>'enabled')::boolean,false);
  if v_controlled then
    return jsonb_build_object('allowed',true,'reason',null,'retryAt',now(),'controlledTestBypass',true,'countsTowardDailyCap',false);
  end if;
  ...
  v_timezone:=coalesce(v_settings->>'timezone','America/Maceio');
  v_now_local:=now() at time zone v_timezone; v_day:=v_now_local::date;
  v_start:=coalesce((v_settings->>'startTime')::time,'08:00'); v_end:=coalesce((v_settings->>'endTime')::time,'22:00');
  v_start_local:=(v_day+v_start)::timestamp; v_end_local:=(v_day+v_end)::timestamp;
  v_campaign_start:=nullif(v_settings->>'campaignStartAt','')::timestamptz;
  ...
  return jsonb_build_object('allowed',true,'reason',null,'retryAt',v_next,'remainingDailyBudget',v_remaining,'intervalSeconds',v_interval,'countsTowardDailyCap',true);
end $$;
...
returns jsonb language sql security definer set search_path=public as $$
  select public.check_outreach_capacity(p_owner,p_lead,p_daily_limit,p_hourly_limit,false);
$$;

revoke all on function public.check_outreach_capacity(uuid,uuid,integer,integer,boolean) from public,anon,authenticated;
grant execute on function public.check_outreach_capacity(uuid,uuid,integer,integer,boolean) to service_role;
