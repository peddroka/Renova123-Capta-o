...
export function appendLatestLeadMessageIfMissing(messages: AgentSnapshot["messages"], latestText: string, createdAt = new Date().toISOString(), latestId?: string) {
  const text = latestText.trim();
  if (!text) return messages;
  if (latestId && messages.some((message) => message.externalId === latestId || message.id === latestId)) return messages;
  let start = messages.length - 1;
  while (start >= 0 && messages[start]?.role === "lead") start -= 1;
  const persistedTurn = messages.slice(start + 1).map((message) => message.text.trim()).filter(Boolean).join("\n");
  const persistedNormalized = normalizeComparable(persistedTurn);
  const textNormalized = normalizeComparable(text);
  if (persistedNormalized === textNormalized || (textNormalized.length > 0 && persistedNormalized.endsWith(textNormalized))) return messages;
...
export function currentLeadTurn(snapshot: AgentSnapshot, fallback: string) {
  ...
  const texts = messages.slice(start + 1).filter((message) => message.role === "lead").map((message) => message.text.trim()).filter(Boolean);
  if (texts.length > 1 && normalizeComparable(texts.at(-1)!) === normalizeComparable(texts.slice(0, -1).join("\n"))) return texts.at(-1)!;
  const compact = texts.filter((text, index) => index === 0 || normalizeComparable(text) !== normalizeComparable(texts[index - 1]!));
  return compact.join("\n").trim() || fallback.trim();
}

export function needsOutboundIdentityRepair(replyText: string | null, snapshot: AgentSnapshot, leadTurn: string) {
  const state = deriveConversationState(snapshot, leadTurn);
  const introductionMissingAfterRoleConfirmation = state.leadRoleKnown && !state.hasIntroduced;
  if (!(state.explicitIdentityQuestion || introductionMissingAfterRoleConfirmation) || !startedAsOutboundProspecting(snapshot)) return false;
  ...
  const explainsReason = /\b(?:entrei em contato|falei com voce|chamei voce|motivo|otica|sistema|gestao)\b/.test(reply);
  ...
  return !identifiesFrancisco || !identifiesCompany || !explainsReason || asksIdentityBack || exposesInternalRole || actsLikeInboundSupport || inventsVoiceCall;
}

export function needsRecentQuestionRepair(replyText: string | null, snapshot: AgentSnapshot, leadTurn: string) {
  if (!replyText || !startedAsOutboundProspecting(snapshot) || !isGreetingOnly(leadTurn)) return false;
  const previousAgent = [...snapshot.messages].reverse().find((message) => message.role === "agent" || message.role === "human");
  ...
  return previousQuestions.some((previous) => replyQuestions.some((current) => similarity(normalizeComparable(previous), normalizeComparable(current)) >= 0.55));
}

export function isGreetingOnly(text: string) {
  const normalized = normalizeComparable(text).replace(/[.!?]+$/g, "").trim();
...
export function naturalMessageParts(text: string) {
  ...
  return paragraphs.length <= 3 ? paragraphs : [paragraphs[0]!, paragraphs[1]!, paragraphs.slice(2).join(" ")];
}
...
export function conversationalBubbleDelayMs(parts: string[], leadTurn: string) {
  ...
  if (/(?:correndo|rapido|urgente|nao tenho interesse|obrigado|nao me chame|para de perguntar|direto ao ponto)/.test(all)) return 0;
  const first = normalizeComparable(parts[0]!);
  const second = normalizeComparable(parts[1]!);
  const hasExplanation = first.split(" ").length >= 8;
  const hasContinuation = /\?|(?:se quiser|posso te mostrar|me conta|hoje voces|voces usam|fica no)/.test(second);
...
export function removeRepeatedFormulations(replyText: string | null, snapshot: AgentSnapshot) {
  ...
  const kept = paragraphParts(replyText).map((paragraph) => sentenceParts(paragraph).filter((sentence) => {
...
export function protectConversationContinuity(replyText: string | null, snapshot: AgentSnapshot, leadTurn: string) {
  if (!replyText) return replyText;
  const state = deriveConversationState(snapshot, leadTurn);
  ...
  if (state.leadRoleKnown) {
    paragraphs = paragraphs.map((parts) => parts.map((part) => removeAnsweredRoleQuestion(part, state.leadNameKnown)).filter(Boolean));
  }
  paragraphs = paragraphs.map((parts) => parts.map(removeCurrentWhatsAppQuestion).filter(Boolean));
  if (volumeAlreadyAnswered(snapshot, leadTurn)) paragraphs = paragraphs.map((parts) => parts.map(removeQuantityQuestion).filter(Boolean));
  return paragraphs.map((parts) => parts.join(" ").replace(/\s+([,.!?;:])/g, "$1").trim()).filter(Boolean).join("\n\n") || null;
}

export function isIrritatedTurn(text: string) {
  const normalized = normalizeComparable(text);
  return /(?:pq tantas perguntas|por que tantas perguntas|de novo a mesma pergunta|ja falei|ja respondi|para de perguntar|nao enche|direto ao ponto|sem enrolacao|cara{2,}|complicad[oa]|e ruim mesmo)/.test(normalized);
}


export function ensureActiveInboundReply(replyText: string | null, options: { shouldHandoff?: boolean; shouldOptOut?: boolean; noInterest?: boolean } = {}) {
  ...
  if (options.shouldHandoff) return "Essa parte eu vou confirmar certinho com o Pedro para não te passar algo errado.";
  return null;
}


function removeCurrentWhatsAppQuestion(value: string) {
  return value.replace(/[^.!?]*(?:qual|confirma|me passa)[^?]{0,80}(?:whatsapp|número|numero)[^?]*\?/gi, "").trim();
}


function removeQuantityQuestion(value: string) {
  return value.replace(/[^.!?]*\b(?:quantos?|quantas?|quanto|volume|n[uú]mero exato|por dia|por m[eê]s)\b[^?]*\?/gi, "").trim();
}

function volumeAlreadyAnswered(snapshot: AgentSnapshot, leadTurn: string) {
  const all = `${leadTurn} ${snapshot.memories.filter((item) => ["answered_questions", "impact"].includes(item.key)).map((item) => item.value).join(" ")}`;
  return /(?:n[aã]o sei (?:o )?n[uú]mero exato|muitos?|bastante|volume (?:alto|n[aã]o quantificado))/.test(normalizeComparable(all));
}
...
function stripSelfIntroduction(value: string) {
  ...
  return value
    .replace(/^(?:(?:oi|olá)[,! ]*)?(?:prazer[,! ]*)?(?:(?:eu )?sou(?: o)?|me chamo|meu nome é|aqui é o?) francisco(?:,? (?:aqui|da renova123|aqui da renova123))?\s*(?:[;.!,-]\s*|$)/i, "")
