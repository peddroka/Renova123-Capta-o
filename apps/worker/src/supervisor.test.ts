...
import {
  ...
  shouldRecoverWorker,
  supervisorTiming,
  workerPidFromInstanceId,
  type WorkerHeartbeat,
} from "./supervisor.js";

const heartbeat = (overrides: Partial<WorkerHeartbeat> = {}): WorkerHeartbeat => ({
  instance_id: "JORDAN:37268:instance",
  status: "running",
  last_heartbeat_at: "2026-08-10T04:00:00.000Z",
  lock_expires_at: "2026-08-10T04:00:30.000Z",
  ...overrides,
});

describe("worker supervisor", () => {
  it("deriva o PID do instance_id sem confiar apenas na existência do processo", () =>
    expect(workerPidFromInstanceId("JORDAN:37268:abc")).toBe(37268));
  ...
  it("reinicia imediatamente quando o worker marcou o lock como stopped", () => {
    const stopped = heartbeat({
      status: "stopped",
      last_heartbeat_at: "2026-08-10T04:00:09.000Z",
      lock_expires_at: "2026-08-10T04:00:09.000Z",
    });
    const now = Date.parse("2026-08-10T04:00:10.000Z");
