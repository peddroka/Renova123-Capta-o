...
export function heartbeatAgeMs(
  heartbeat: Pick<WorkerHeartbeat, "last_heartbeat_at"> | null,
  now = Date.now(),
) {
...
async function latestHeartbeat(db: SupabaseClient, owner: string) {
  const result = (await db
    ...
    .select("instance_id,status,last_heartbeat_at,lock_expires_at")
    .eq("owner_id", owner)
    .eq("worker_type", "main")
    .order("last_heartbeat_at", { ascending: false })
    .limit(1)
    .maybeSingle()) as { data: WorkerHeartbeat | null; error: Error | null };
...
export async function runSupervisor() {
  if (!workerConfig.SUPABASE_URL || !workerConfig.SUPABASE_SERVICE_ROLE_KEY)
    throw new Error("Supervisor requer Supabase persistente.");
  const db = createClient(workerConfig.SUPABASE_URL, workerConfig.SUPABASE_SERVICE_ROLE_KEY, {
  ...
  while (!stopping) {
    try {
      ...
      console.error("worker_supervisor_cycle_failed", error);
    }
