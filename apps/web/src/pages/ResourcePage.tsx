...
export function ResourcePage({ pageKey }: { pageKey: PageKey }) {
  ...
  const columns = useMemo(() => pageKey === "queue" ? ["type", "status", "presenceStatus", "priority", "availableAt", "attempts", "queueName"].filter((key) => Object.prototype.hasOwnProperty.call(result?.rows[0] ?? {}, key)) : Object.keys(result?.rows[0] ?? {}).filter((key) => key !== "id" && !["lastError", "details"].includes(key)).slice(0, pageKey === "openers" ? 5 : 7), [result, pageKey]);
  ...
  return <div className="page-stack">
    ...
    <section className="card data-card">
      ...
      {loading ? <SkeletonTable /> : !result?.rows.length ? <EmptyState title="Nada por aqui ainda" description="Quando a operação gerar registros, eles aparecerão nesta área com os filtros aplicados." /> : <div className="table-scroll"><table><thead><tr>{columns.map((column) => <th key={column}>{labels[column] ?? titleCase(column)}</th>)}{config ? <th aria-label="Ações" /> : null}</tr></thead><tbody>{result.rows.map((row, index) => <tr key={String(row.id ?? index)}>{columns.map((column) => <td key={column}>{formatCell(column, row[column])}</td>)}{config ? <td className="row-actions">{pageKey === "queue" ? <><button aria-label="Tentar novamente" title="Tentar novamente" onClick={() => void retry(row)}><RotateCcw /></button><button aria-label="Cancelar item" title="Cancelar item" onClick={() => void cancelQueueItem(row)}><X /></button></> : null}{pageKey === "conversations" ? <button aria-label="Enviar mensagem manual" title="Enviar mensagem manual" onClick={() => void manualMessage(row)}><MessageSquareText /></button> : null}{pageKey === "leads" ? <WhatsAppLink phone={row.phone} /> : null}{config.edit ? <button aria-label="Editar" title="Editar" onClick={() => setEditing(row)}><Pencil /></button> : null}{config.remove ? <button aria-label="Excluir" title="Excluir" onClick={() => void remove(row)}><Trash2 /></button> : null}</td> : null}</tr>)}</tbody></table></div>}
...
function friendly(key: PageKey) { return ({ leads: "leads", batches: "lotes", queue: "fila", conversations: "conversas", interested: "interessados", demos: "demonstrações", unanswered: "não responderam", followups: "follow-ups", handoffs: "transferidos", lost: "perdidos", optouts: "bloqueios", openers: "mensagens", logs: "auditoria", health: "serviços" } as Partial<Record<PageKey, string>>)[key] ?? "registros"; }
