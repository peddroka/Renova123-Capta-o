...
const defaults: Record<string, Record<string, unknown>> = {
  outreach: { dailyLimit: 50, dailyProactiveLimit: 50, hourlyLimit: 8, weekdays: [0, 1, 2, 3, 4, 5, 6], startTime: "08:00", endTime: "23:00", minIntervalMinutes: 7, maxIntervalMinutes: 16, proactiveHardFloorMinutes: 6, proactiveJitterMinMinutes: 1, proactiveJitterMaxMinutes: 10, minIntervalSeconds: 5, maxIntervalSeconds: 5, timezone: "America/Sao_Paulo", maxConsecutiveFailures: 5, autoPause: true, followUpsEnabled: true, maxFollowUps: 3, followUpIntervalHours: 48, batchPriority: "priority", demoDays: "segunda a sexta", demoStartTime: "09:00", demoEndTime: "17:00", demoDurationMinutes: 45, demoBufferMinutes: 15, demoMinNoticeHours: 24, demoBlockedSlots: "", demoCloser: "", demoNotes: "" },
...
const fieldSets: Record<string, Field[]> = {
  outreach: [
    ...
    { key: "proactiveHardFloorMinutes", label: "Bloqueio mínimo absoluto (minutos)", type: "number", help: "Nunca pode ser menor que 6. Depois deste bloqueio o sistema ainda aplica um jitter aleatório." },
    { key: "proactiveJitterMinMinutes", label: "Jitter adicional mínimo (minutos)", type: "number", help: "Aplicado somente após o bloqueio absoluto." },
    { key: "proactiveJitterMaxMinutes", label: "Jitter adicional máximo (minutos)", type: "number", help: "Com 6 + 1–10, o próximo contato sai entre 7 e 16 minutos depois." },
