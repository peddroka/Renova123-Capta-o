...
function bestHourLabel(hour: number | null, analytics: OutreachAnalytics) {
  if (hour === null)
    return `Dados insuficientes (mínimo ${analytics.minimumSampleSize} contatos por horário)`;
...
function PerformanceChart({ hours }: { hours: OutreachHourMetric[] }) {
  ...
  const points = (field: "responseRate" | "qualificationRate") =>
    hours.map((item, index) => `${x(index)},${y(item[field])}`).join(" ");
  return (
    <div className="performance-chart" aria-label="Gráfico de taxa de resposta e qualificação por hora">
      <svg viewBox={`0 0 ${width} ${height}`} role="img">
        ...
        {[0, 0.5, 1].map((value) => (
          <g key={value}>
            ...
            />
            <text x={2} y={y(value) + 3} className="performance-axis-label">
              {percentage(value)}
            </text>
          </g>
        ))}
        ...
        <polyline
          points={points("qualificationRate")}
          className="performance-line performance-line--qualification"
        />
        {hours.map((item, index) => (
          <g key={item.hour}>
            <circle
              cx={x(index)}
              cy={y(item.responseRate)}
              r="3"
              className="performance-dot performance-dot--response"
            >
              <title>
                {item.label}: resposta {percentage(item.responseRate)}, {item.responded}/{item.sent}
              </title>
            </circle>
            <circle
              cx={x(index)}
              cy={y(item.qualificationRate)}
              r="3"
              className="performance-dot performance-dot--qualification"
            >
              <title>
                {item.label}: qualificação {percentage(item.qualificationRate)}, {item.qualified}/{item.sent}
              </title>
            </circle>
            {index % 2 === 0 ? (
              <text x={x(index)} y={height - 8} textAnchor="middle" className="performance-axis-label">
                {item.label}
              </text>
            ) : null}
          </g>
      ...
      <div className="performance-legend">
        <span>
          <i className="performance-key performance-key--response" />
          Resposta
        </span>
        <span>
          <i className="performance-key performance-key--qualification" />
          Qualificação
        </span>
      </div>
    </div>
...
function OutreachPerformance({ analytics }: { analytics: OutreachAnalytics }) {
  return (
    <DataCard
      ...
      description={`Primeira abordagem em ${analytics.timezone}. Respostas são atribuídas ao horário do envio inicial.`}
    >
      <PerformanceChart hours={analytics.hours} />
      <div className="performance-highlights">
        <div>
          <span>Melhor horário por resposta</span>
          <strong>{bestHourLabel(analytics.bestResponseHour, analytics)}</strong>
        </div>
        <div>
          <span>Melhor horário por qualificação</span>
          <strong>{bestHourLabel(analytics.bestQualificationHour, analytics)}</strong>
        </div>
        <div>
          <span>Tamanho da amostra</span>
          <strong>{analytics.totalSample} contatos</strong>
        </div>
      </div>
          ...
          <tbody>
            {analytics.hours.map((item) => (
              <tr key={item.hour}>
                ...
                <td>{percentage(item.responseRate)}</td>
                <td>{item.qualified}</td>
                <td>{percentage(item.qualificationRate)}</td>
                <td>
                  {item.medianMinutesToFirstResponse === null
                    ? "—"
                    : `${item.medianMinutesToFirstResponse} min`}
                </td>
              </tr>
            ))}
          </tbody>
  ...
  async function saveNewLeadLimit() {
    ...
    try {
      await api("/settings/outreach", {
        ...
        body: JSON.stringify({
          ...data.outreach,
          newLeadsDailyLimit: Math.max(1, Math.min(500, newLeadLimit)),
  ...
  const metrics: Metric[] = stats
    ? [
        ...
        },
        {
          ...
          note: "abordagens registradas",
          icon: PhoneCall,
        },
        {
          id: "limit",
          label: "Contatados hoje / limite",
          value: `${stats.contactedToday}/${stats.dailyLimit}`,
          note: "disparos proativos",
          icon: Gauge,
        },
        {
          id: "queue",
        ...
        },
        {
          ...
          note: "atendimento humano",
          icon: Bot,
          tone: stats.handoffs ? "warning" : "positive",
        },
        { id: "optouts", label: "Opt-outs", value: stats.optOuts, note: "bloqueios ativos", icon: Ban },
        {
          id: "mode",
          label: "Modo de envio",
          value: stats.sendMode ?? (stats.simulationMode ? "MOCK / BLOQUEADO" : "REAL / PAUSADO"),
          note: stats.sendModeReason ?? "estado calculado pelo backend",
          icon: AlertTriangle,
          tone: "warning",
        },
      ]
    ...
    <div className="page-stack dashboard-page">
      <PageHeader
        ...
        actions={
          <div className="hero-actions">
            ...
            <Link className="hero-button" to="/conversas">
      ...
      <section className="dashboard-metrics" aria-label="Indicadores reais da operação">
        {loading
          ? Array.from({ length: 10 }, (_, index) => (
              <MetricCard key={index} label="Carregando" value="" note="" icon={<UsersRound />} loading />
            ))
          : metrics.map((metric) => {
      ...
      <section className="dashboard-primary-grid">
        ...
        <DataCard
          ...
          action={
            <Link className="inline-link" to="/saude">
              Diagnóstico
            </Link>
          }
        >
                ...
                return (
                  <SystemStatusIndicator
                    ...
                    tone={
                      good
                        ? "positive"
                        : state === "mock" || state === "simulation" || state === "not_created"
                          ? "warning"
                          : "danger"
                    }
                  />
      ...
      {!loading && data ? (
        <section className="dashboard-analytics">
          <OutreachPerformance analytics={data.outreachAnalytics} />
        </section>
      ) : null}
      ...
      <section className="dashboard-triple-grid">
        <DataCard
          ...
          action={
            <Link className="inline-link" to="/logs">
              Ver logs
            </Link>
          }
        >
          {loading ? (
            ...
            <div className="simple-list">
              {data.logs.map((item) => (
                <div key={item.id}>
                  <span>
                    <strong>{item.action ?? item.eventType ?? "Evento"}</strong>
                    <small>{item.service ?? "sistema"}</small>
                  </span>
            ...
            <EmptyState title="Sem atividade recente" description="Os eventos reais aparecerão aqui." />
          )}
        </DataCard>
        <DataCard
          eyebrow="Agenda"
          ...
          action={
            <Link className="inline-link" to="/demonstracoes">
              Abrir agenda
            </Link>
          }
        >
          {loading ? (
            ...
            <div className="simple-list">
              {data.appointments.map((item) => (
                <div key={item.id}>
                  <span>
                    <strong>{item.lead ?? item.leadId ?? "Lead"}</strong>
                    <small>{item.company ?? item.assignee ?? "Sem responsável"}</small>
                  </span>
            ...
            <EmptyState title="Agenda livre" description="Nenhuma demonstração real agendada." />
          )}
        </DataCard>
        <DataCard
