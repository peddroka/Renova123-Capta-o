...
describe("repositório mock persistente", () => {
  it("repete somente leitura após reset de socket e nunca repete POST", async () => {
    ...
    await expect(supabaseFetchWithRetry("https://supabase.test/rest/v1/leads")).resolves.toMatchObject({
      status: 200,
    });
    await expect(
      supabaseFetchWithRetry("https://supabase.test/rest/v1/rpc/send", { method: "POST" }),
  ...
  it("compartilha lotes e fila entre API e worker", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "renova-captacao-"));
    ...
    const result = await apiRepository.createBatch(
      {
        name: "Lote teste",
        source: "Teste autorizado",
    ...
    const workerRepository = createRepository({
      mock: true,
      supabaseUrl: undefined,
      serviceRoleKey: undefined,
      mockFilePath: file,
  ...
  it("libera apenas a primeira abordagem quando chega presença online", async () => {
    ...
    const result = await repository.createBatch(
      {
        name: "Online only",
        source: "Captação comercial",
        startDate: new Date().toISOString(),
        initialStrategy: "Mensagem inicial",
      },
      ["5511912345678"],
    );
  ...
  it("mantém a reserva de limite no retry da mesma abordagem", async () => {
    ...
    await repository.deferOutreachWithPayload(job.id, new Date(), "falha transitória", {
      ...job.payload,
      capacityReservedAt: true,
    });
  ...
  it("persiste evento inbound duplicado sem duplicar mensagem nem conversa", async () => {
    ...
    const event = {
      phone: "5511912345678",
      externalMessageId: "wamid-final-1",
      eventId: "evt-final-1",
      text: "Oi",
  ...
  it("faz claim exclusivo entre duas instâncias e respeita cancelamento", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "renova-claim-"));
    directories.push(directory);
    const file = path.join(directory, "mock-db.json");
    const firstWorker = createRepository({
      mock: true,
      supabaseUrl: undefined,
      serviceRoleKey: undefined,
      mockFilePath: file,
    });
    const secondWorker = createRepository({
      mock: true,
      supabaseUrl: undefined,
      serviceRoleKey: undefined,
      mockFilePath: file,
    });
    const claimable = await firstWorker.enqueue(
      "inbound_reply",
      { phone: "5511911112222", text: "oi" },
      new Date(),
      "in:1",
    );
    const cancelled = await firstWorker.enqueue(
      "follow_up",
      { leadId: "lead-2" },
      new Date(),
      "follow:cancel",
    ...
    const [firstClaim, secondClaim] = await Promise.all([
      firstWorker.claimJobs(10),
      secondWorker.claimJobs(10),
    ]);
  ...
  it("preserva escritas de duas instâncias sem uma sobrescrever a outra", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "renova-lock-"));
    directories.push(directory);
    const file = path.join(directory, "mock-db.json");
  ...
  it("impede importação de opt-out e pagina resultados", async () => {
    ...
    const result = await repository.createBatch(
      ...
      ["5511999990000", "5511912345678", "5511912345679", "5511912345680"],
    );
  ...
  it("distribui novas abordagens na janela e prioriza respostas", async () => {
    ...
    const result = await repository.createBatch(
      ...
      Array.from({ length: 100 }, (_, index) => `551191234${String(index).padStart(4, "0")}`),
    );
    ...
    await repository.enqueue(
      "inbound_reply",
      { phone: "5511999999999", text: "respondeu" },
      new Date(),
      "inbound:old-lead",
    );
  ...
  it("recupera inbound pendente uma vez sem criar job duplicado", async () => {
    ...
    const first = await repository.enqueueInboundDebounced(
      { phone: "5511992468815", text: "Sou eu", messageId: "inbound-8815" },
      new Date(),
    );
    const reconciled = await repository.enqueueInboundDebounced(
      { phone: "5511992468815", text: "Sou eu", messageId: "inbound-8815" },
      new Date(),
    );
  ...
  it("preserva outbound sem claim durante pausa operacional", async () => {
    ...
    await repository.enqueue(
      "outreach",
      { phone: "5511992468815", leadId: "real" },
      new Date(),
      "outreach:paused-real",
    );
  ...
  it("continua priorizando inbound quando prospecção está pausada", async () => {
    ...
    await repository.enqueue(
      ...
      { phone: "5511992468815", leadId: "real" },
      new Date(),
      "outreach:paused-proactive",
    );
    await repository.enqueue(
      "inbound_reply",
      { phone: "5511992468816", text: "Oi", messageId: "inbound-24h" },
      new Date(),
      "inbound:24h",
    );
  ...
  it("inbound já respondido não reaparece como recovery", async () => {
    ...
    const id = await repository.enqueueInboundDebounced(
      { phone: "5511974442893", text: "Sim", messageId: "inbound-2893" },
      new Date(),
    );
    expect((await repository.claimJobs(1))[0]?.id).toBe(id);
    await repository.completeJob(id);
  ...
  it("reenfileira o inbound mais novo enquanto a decisão anterior está processando", async () => {
    ...
    const first = await repository.enqueueInboundDebounced(
      { phone: "5511974442893", text: "Costumo", messageId: "inbound-a" },
      new Date(),
    );
    expect((await repository.claimJobs(1))[0]?.id).toBe(first);
    const second = await repository.enqueueInboundDebounced(
      { phone: "5511974442893", text: "Difícil mesmo no dia a dia", messageId: "inbound-b" },
      new Date(),
    );
  ...
  it("consolida inbounds rápidos ainda pendentes em um único job", async () => {
    ...
    const first = await repository.enqueueInboundDebounced(
      { phone: "5511974442893", text: "Sim", messageId: "inbound-c" },
      new Date(),
    );
    const merged = await repository.enqueueInboundDebounced(
      { phone: "5511974442893", text: "eu mesmo", messageId: "inbound-d" },
      new Date(),
    );
  ...
  it("cria novo processamento depois de uma resposta já concluída", async () => {
    ...
    const first = await repository.enqueueInboundDebounced(
      { phone: "5511974442894", text: "A", messageId: "inbound-e" },
      new Date(),
    );
    await repository.completeJob((await repository.claimJobs(1))[0]!.id);
    const second = await repository.enqueueInboundDebounced(
      { phone: "5511974442894", text: "B", messageId: "inbound-f" },
      new Date(),
    );
    expect(second).not.toBe(first);
  ...
  it("mantém um único processamento para evento duplicado persistido", async () => {
    ...
    expect((await repository.persistInboundEvent(event)).inserted).toBe(false);
    const first = await repository.enqueueInboundDebounced(
      { phone: event.phone, text: event.text, messageId: event.externalMessageId },
      new Date(),
    );
    const duplicate = await repository.enqueueInboundDebounced(
      { phone: event.phone, text: event.text, messageId: event.externalMessageId },
      new Date(),
    );
    expect(duplicate).toBe(first);
  ...
  it("preserva o inbound mais novo quando o primeiro processamento é adiado", async () => {
    ...
    const first = await repository.enqueueInboundDebounced(
      { phone: "5511974442896", text: "A", messageId: "inbound-g" },
      new Date(),
    );
    ...
    await repository.enqueueInboundDebounced(
      { phone: "5511974442896", text: "B", messageId: "inbound-h" },
      new Date(),
    );
    const jobs = await repository.claimJobs(10);
  ...
  it("não cria outbound automático para um inbound bloqueado por estado humano", async () => {
    ...
    const lead = await repository.persistInboundEvent({
      phone: "5511974442897",
      externalMessageId: "inbound-human",
      eventId: "evt-human",
      text: "Não",
      occurredAt: new Date().toISOString(),
      messageType: "text",
