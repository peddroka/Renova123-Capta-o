...
export const aiDecisionSchema = z.object({
  ...
  memoryUpdates: z.array(z.object({
    key: z.enum(["informed_name", "store_name", "store_count", "city", "current_system", "current_system_type", "current_process", "main_pain", "relevant_secondary_pain", "impact", "decision_maker", "professional_category", "interest", "interest_signals", "irritation_signals", "objections", "urgency", "budget", "preferred_tone", "current_topic", "answered_questions", "asked_topics", "last_useful_question", "sent_materials", "availability", "demo_status", "demo_discussed", "demo_accepted", "marilia_explained", "loss_reason", "next_action"]),
  ...
  appointmentData: z.object({
    startsAt: z.string().datetime({ offset: true }).describe("Data/hora ISO 8601 completa com fuso, nunca texto relativo."),
    endsAt: z.string().datetime({ offset: true }).describe("Data/hora ISO 8601 completa com fuso, nunca texto relativo."),
    notes: z.string().max(1000),
  }).nullable(),
  shouldHandoff: z.boolean(),
  handoffReason: z.string().max(500).nullable(),
  shouldOptOut: z.boolean(),
  followUpAction: z.object({ action: z.enum(["schedule", "cancel", "none"]), delayHours: z.number().int().min(1).max(720).nullable(), reason: z.string().max(500) }),
  confidence: z.number().min(0).max(1),
  ...
  handoffType: z.enum(["sales_qualified", "human_requested", "low_confidence", "technical", "pricing", "other"]).nullable(),
  ...
  action: z.enum(["continue_discovery", "explain_product", "answer_question", "create_curiosity", "offer_demo", "confirm_demo", "handoff", "close_disinterest"]).optional(),
}).strict();
...
export const outreachSettingsSchema = z.object({
  ...
  if (value.startTime >= value.endTime) context.addIssue({ code: "custom", path: ["endTime"], message: "O horário final deve ser posterior ao inicial." });
  if (value.minIntervalSeconds > value.maxIntervalSeconds) context.addIssue({ code: "custom", path: ["maxIntervalSeconds"], message: "O intervalo máximo deve ser maior ou igual ao mínimo." });
});
