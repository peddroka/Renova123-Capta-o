...
const CORE_INSTRUCTION = [
  ...
  "Em prospecção outbound, se perguntarem quem fala, diga nome, empresa e motivo comercial; retribua naturalmente saudações e perguntas sociais. Se o lead apenas cumprimentar após uma pergunta sua, isso não responde a pergunta: não marque decisor e não repita a mesma pergunta no turno seguinte; apresente-se, explique brevemente o motivo do contato e retome a informação depois. Nunca faça duas perguntas semanticamente iguais em turnos consecutivos. Permissão para continuar não é interesse. Pedido de demo, contratação, preço ou horário após aceitar demo é intenção comercial forte: com decisor e dor já conhecidos, pare de entrevistar e avance para handoff.",
...
export class AgentContextBuilder {
  ...
  build(snapshot: AgentSnapshot, userMessage: string): BuiltAgentContext {
    ...
    const memories = this.memory.select(snapshot.memories, 48)
      .filter((item) => !containsOperationalMetadata(item.value))
      .map((item) => ({ key: item.key, value: item.value.slice(0, 240), ...(item.evidenceType ? { evidenceType: item.evidenceType } : {}) }));
    const recentMessages = groupRecentTurns(recentHistoryExcludingCurrentTurn(snapshot.messages, userMessage), 6)
    ...
    const relevantLibrary = (shouldRetrieve ? deduplicateKnowledge(relevantKnowledge(snapshot.knowledgeItems ?? [], knowledgeQuery, snapshot.stage)) : [])
      .slice(0, knowledgeLimit)
      .map((item) => clipRecord({ ...pick(item, ["title", "category", "subject", "content"]), capabilityStatus: capabilityStatus(item) }, 360));
    const mindKnowledge = shouldRetrieve ? this.knowledge.select(snapshot.mind, snapshot.commercial, knowledgeQuery, 700) : {};
    const contextualKnowledge = relevantLibrary.length
      ? pick(mindKnowledge, ["prices", "commercialTerms", "exceptions", "authorizationRequired", "objections", "approvedAnswers"])
      : mindKnowledge;
    const productTurn = isProductTurn(userMessage, state);
    const demoTurn = state.demoInterest || /\b(demo|demonstra[cç][aã]o|hor[aá]rio|agenda|agendar)\b/i.test(userMessage);
    const materialTurn = /\b(material|pdf|arquivo|folder|apresenta[cç][aã]o|manda|envia)\b/i.test(userMessage);
    const socialProofTurn = /\b(clientes?|[oó]ticas? atendidas?|prova social|macei[oó]|alagoas)\b/i.test(userMessage);
    const sourceTurn = /\b(como|onde|de onde).{0,30}(n[uú]mero|contato)|conseguiu meu (?:n[uú]mero|contato)|como sabe meu nome/i.test(userMessage);
    const informedName = memories.find((item) => item.key === "informed_name" && item.evidenceType !== "hypothesis")?.value;

    const selected: Record<string, any> = {
      mind: compileMind(snapshot.mind, snapshot.commercial),
      truth: pick(CONFIRMED_PRODUCT_CATALOG.overview as unknown as Record<string, unknown>, ["product", "benefit"]),
      ...(memories.length ? { memory: memories } : {}),
      ...(recentMessages.length ? { recentMessages } : {}),
      conversation: compactConversation(snapshot, plan, state),
      ...(contextualKnowledge && Object.keys(contextualKnowledge).length ? { knowledge: contextualKnowledge } : {}),
      ...
      ...(materialTurn ? { materials: snapshot.materials.filter((item) => item.active && !item.alreadySent).slice(0, 2).map((item) => pick(item as unknown as Record<string, unknown>, ["id", "name", "description", "instruction"])) } : {}),
      ...
      ...((informedName || snapshot.lead.company || snapshot.lead.city) ? { lead: commercialLead(snapshot.lead, informedName) } : {}),
      ...((snapshot.humanActive || snapshot.automationPaused || snapshot.blocked) ? { controls: { humanActive: snapshot.humanActive, automationPaused: snapshot.automationPaused, blocked: snapshot.blocked } } : {}),
    };
    ...
    const blocks = {
      ...
      knowledge: { knowledge: selected.knowledge, relevantKnowledge: selected.relevantKnowledge, productBase: selected.productBase },
    ...
    return { systemPrompt, selected, estimatedTokens: tokenBreakdown.systemTokens, summarized: snapshot.messages.length > recentMessages.length, tokenBreakdown };
  }
}

function compactConversation(snapshot: AgentSnapshot, plan: ReturnType<typeof planConversation>, state: ReturnType<typeof deriveConversationState>) {
  const signals = Object.fromEntries(Object.entries({
  ...
  const qualification = {
    ...(snapshot.qualificationStatus && snapshot.qualificationStatus !== "discovering" ? { status: snapshot.qualificationStatus } : {}),
    ...(snapshot.qualificationScore ? { score: snapshot.qualificationScore } : {}),
    ...(snapshot.handoffType ? { handoffType: snapshot.handoffType } : {}),
    ...(snapshot.mariliaConsent && snapshot.mariliaConsent !== "not_asked" ? { mariliaConsent: snapshot.mariliaConsent } : {}),
  };
  return {
    origin: state.origin,
    ...
    ...(plan.unansweredUserQuestion ? { pendingQuestion: String(plan.unansweredUserQuestion).slice(0, 240) } : {}),
    ...(plan.answeredTopics.length ? { answered: plan.answeredTopics.slice(-6) } : {}),
    ...(snapshot.questionsAsked.length ? { asked: snapshot.questionsAsked.slice(-4).map((question) => question.slice(0, 240)) } : {}),
...
function compileMind(mind: Record<string, unknown>, commercial: Record<string, unknown>) {
  ...
  return Object.fromEntries(MIND_GROUPS.flatMap(([group, keys]) => {
...
function recentHistoryExcludingCurrentTurn(messages: AgentMessage[], userMessage: string) {
  ...
  const parts = userMessage.split(/\n+/).map((part) => normalize(part.trim())).filter(Boolean);
...
function compactOlderSummary(summary: string, messageCount: number, memoryCount: number) {
  ...
  const parts = current.split(/(?<=[.!?])\s+/).filter((part) => memoryCount === 0 || !/^Fatos comerciais:/i.test(part));
...
function enforceConceptualBudget(selected: Record<string, any>, tokenLimit: number) {
  ...
  while (size() > tokenLimit && selected.memory?.some((item: { evidenceType?: string }) => item.evidenceType !== "explicit")) {
    const index = selected.memory.findIndex((item: { evidenceType?: string }) => item.evidenceType !== "explicit");
    selected.memory.splice(index, 1);
  }
}

function pick(source: Record<string, unknown>, keys: string[]) {
  return Object.fromEntries(keys.flatMap((key) => source?.[key] === undefined || source?.[key] === "" ? [] : [[key, source[key]]]));
}

function isProductTurn(message: string, state: ReturnType<typeof deriveConversationState>) {
  return state.productCuriosity || state.demoInterest || /(?:sistema|recurso|funciona|or[cç]amento|estoque|financeiro|cliente|produto|medi[cç][aã]o|pupilar|medidor|\bdp\b|pre[cç]o|valor)/i.test(message);
}
...
function knowledgeQueryForTurn(userMessage: string, history: Array<{ role: string; text: string }>) {
  ...
  const lastAgent = [...history].reverse().find((message) => message.role === "agent" || message.role === "human")?.text;
  return lastAgent?.includes("?") ? `${lastAgent}\n${userMessage}` : userMessage;
}

function shouldRetrieveKnowledge(message: string) {
  return /\b(sistema|produto|produtos|cliente|clientes|cadastro|estoque|financeiro|contas?|recebimentos?|caixa|medidor|dp|medi[cç][aã]o|pupilar|or[cç]amento|proposta|pre[cç]o|caro|planilha|papel|caneta|material|pdf|demo|demonstra[cç][aã]o|integra[cç][aã]o|m[oó]dulo|funcionalidade|compra|retorno|prescri[cç][aã]o)\b/i.test(message);
}
...
function commercialProvenance(lead: Record<string, unknown>, batch: Record<string, unknown>) {
  const provenance = pick({ source: lead.source, batchSource: batch.source, batchContext: batch.context }, ["source", "batchSource", "batchContext"]);
  return Object.fromEntries(Object.entries(provenance).filter(([, value]) => !containsOperationalMetadata(value)));
}

function containsOperationalMetadata(value: unknown) {
  return typeof value === "string" && /\b(test|teste|authorized test number|lista de teste|whitelist|simulation|simulacao|codex|pipeline|jobs?|fila|flag|ambiente de desenvolvimento|autorizad[oa] para teste)\b/.test(normalize(value));
}
...
function relevantKnowledge(items: Array<Record<string, unknown>>, userMessage: string, _stage: string) {
  ...
  const ignored = new Set(["ainda", "maioria", "encontro", "bacana", "como", "posso", "ajudar", "gostaria", "quero", "isso", "essa", "esse", "para", "pela", "pelo", "qual"]);
  const terms = query.split(/\s+/).filter((term) => (term.length >= 3 || term === "dp") && !ignored.has(term));
  ...
  const aliases = Object.values(topics).filter((words) => words.some((word) => query.includes(word))).flat();
  const ranked = items.filter((item) => item.active !== false && capabilityStatus(item) !== "UNCONFIRMED").map((item, index) => {
    const searchable = normalize([item.title, item.category, item.subject, item.tags, item.stages, item.content].map(String).join(" "));
    ...
    const directScore = Object.values(topics).flat().reduce((score, term) => score + (directQuery.includes(term) && searchable.includes(term) ? 8 : 0), 0)
      ...
      + (/(?:\bdp\b|medidor)/.test(directQuery) && /(?:solucao apresenta|capacidade confirmada|oferece|permite)/.test(searchable) ? 10 : 0);
    return { item, index, relevance: termHits + topicScore + directScore };
  }).filter(({ relevance }) => relevance > 0).sort((left, right) => right.relevance - left.relevance || left.index - right.index);
...
function deduplicateKnowledge(items: Array<Record<string, unknown>>) {
  ...
  for (const item of items) {
    const tokens = new Set(normalize(`${item.title ?? ""} ${item.content ?? ""}`).split(/\s+/).filter((token) => token.length >= 5));
    const duplicate = accepted.some((other) => {
      const otherTokens = new Set(normalize(`${other.title ?? ""} ${other.content ?? ""}`).split(/\s+/).filter((token) => token.length >= 5));
      const overlap = [...tokens].filter((token) => otherTokens.has(token)).length;
      return overlap >= 8 && overlap / Math.max(1, Math.min(tokens.size, otherTokens.size)) >= 0.55 || Boolean(item.category && item.category === other.category && overlap >= 5);
    });
...
function clipRecord(source: Record<string, unknown>, maxTextLength: number) {
  return Object.fromEntries(Object.entries(source).map(([key, value]) => [key, typeof value === "string" ? value.slice(0, maxTextLength) : value]));
}
...
function locationHint(lead: Record<string, unknown>) {
  ...
  return ddd ? { ddd, probableState: ddd === "82" ? "Alagoas" : null, confidence: "probable_region_only", cityConfirmed: false } : null;
}
